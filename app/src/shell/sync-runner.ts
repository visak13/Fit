 ...
 * It is one of the five declared opportunities (`interval`) and nothing else.
 *
 ...
 * the coach's own quota, on a phone that is probably in his hand mid-session. The four other
 * opportunities — open, foreground, leave and his own tap — already cover every moment when something
 * has plausibly changed; this one exists for the long single sitting where nothing else fires.
 *
 ...
 * it could not read** — the last of which is the false green: an older installation that finds every
 * file the newer one wrote undecodable holds none of its work, and must not show green.
 *
...
export async function readSyncReading(
  ...
  const status = await accountabilityStatus(store, {
    in_progress: options.inProgress,
    last_attempt: options.lastAttempt,
    credential: options.credential,
  });
  return status as unknown as SyncStatusReading;
}

/**
 * THE WORLD THE OPPORTUNITIES ARE OBSERVED IN — every global this module needs, as four functions.
 *
 ...
 * check. With this, every one of the five opportunities can be fired and asserted with no browser.
 *
 * Each subscription returns its own removal, so nothing has to remember which listener went with
 * which surface — the document's visibility and the window's page-hide are two different objects in
 * the browser and one shape here.
 */
 ...
 *
 * `SYNC_TRIGGERS` in `core/sync/engine.js` declares the five as data, and `syncNow` REFUSES a trigger
 * that is not one of them, so this cannot quietly invent a sixth. Four of them are observed here; the
 * fifth is the coach's own tap and arrives through the actions rather than through a listener.
 *
